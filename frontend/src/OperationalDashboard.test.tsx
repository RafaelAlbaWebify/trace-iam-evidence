...
test("shows operational workload, applies filters, and opens by generated ID", async () => {
  ...
  expect(screen.getByText("1", { selector: ".operations-metrics strong" })).toBeInTheDocument();
  ...
  await waitFor(() => expect(String(fetchMock.mock.calls.at(-1)?.[0])).toContain("query=INC-CRIT-7"));
  expect(String(fetchMock.mock.calls.at(-1)?.[0])).toContain("priority=critical");
