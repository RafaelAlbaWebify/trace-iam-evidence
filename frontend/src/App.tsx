...
function nextLifecycleAction(status: InvestigationStatus): { status: InvestigationStatus; label: string } | null {
  if (status === "draft") return { status: "evidence_validated", label: "Mark evidence validated" };
...
export function App() {
  ...
  function loadEditor(detail: InvestigationDetail) {
    ...
    setEditReference(detail.external_reference ?? "");
  ...
  async function createCase(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const created = await api<InvestigationDetail>("/api/investigations", {
        ...
        body: JSON.stringify({
          title: caseTitle,
          scenario_type: caseScenario,
          priority: casePriority,
          external_reference: caseReference || null,
      ...
      setRuns([]);
      setNotice(`Investigation ${created.investigation_id} created and selected.`);
  ...
  async function activateCase(item: InvestigationSummary) {
    ...
    try {
      ...
      loadEditor(detail);
      setSelectedId(detail.investigation_id);
      setRuns(await api<AnalysisRun[]>(`/api/investigations/${detail.investigation_id}/runs`));
      setNotice(`Investigation ${detail.investigation_id} is now active.`);
      document.getElementById("active-case-control")?.scrollIntoView({ behavior: "smooth", block: "start" });
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "Investigation failed to load");
    }
  }
  ...
  async function updateCaseMetadata(event: FormEvent<HTMLFormElement>) {
    ...
    setNotice("");
    try {
      const updated = await api<InvestigationDetail>(`/api/investigations/${activeCase.investigation_id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          title: editTitle,
          priority: editPriority,
          external_reference: editReference || null,
          summary: editSummary || null
        })
      });
      setActiveCase(updated);
      loadEditor(updated);
      setNotice("Case metadata saved without changing immutable analysis runs.");
      await refreshHistory();
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "Case metadata update failed");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function transitionCase(target: InvestigationStatus) {
    ...
    setNotice("");
    try {
      const updated = await api<InvestigationDetail>(`/api/investigations/${activeCase.investigation_id}/transition`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      setActiveCase(updated);
      loadEditor(updated);
      setNotice(`Lifecycle moved to ${updated.status}.`);
      await refreshHistory();
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "Lifecycle transition failed");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function completeAnalysis(payload: AnalysisResponse) {
    ...
    if (activeCase) {
      const updated = { ...activeCase, status: "analyzed" as const, analysis_run_count: payload.run_number };
      setActiveCase(updated);
      loadEditor(updated);
    }
  ...
  async function runAnalysis(path: string, body: Record<string, unknown>) {
    if (!activeCase) {
      setError("Create or select an investigation before analyzing evidence.");
      return;
    }
    setLoading(true);
    setError("");
  ...
  async function submitConditionalAccess(event: FormEvent<HTMLFormElement>) {
    ...
    await runAnalysis("/api/investigations/analyze-conditional-access-csv", {
      ...
      source: "public-safe operator evidence",
      csv_text: csvText
    });
  }
  ...
  async function changeArchiveState(investigationId: string, action: "archive" | "reopen") {
    ...
    setNotice("");
    try {
      const updated = await api<InvestigationDetail>(`/api/investigations/${investigationId}/${action}`, { method: "POST" });
      await refreshHistory();
      if (activeCase?.investigation_id === investigationId) {
        if (action === "archive") setActiveCase(null);
        else {
          setActiveCase(updated);
          loadEditor(updated);
        }
  ...
  return (
    <main className="app-shell" aria-busy={unavailable}>
      <header className="hero">
        <p className="eyebrow">Local-first IAM investigation workbench</p>
        <h1>TRACE IAM Evidence</h1>
        <p>Create operational cases, preserve redacted metadata, execute deterministic rules, and retain immutable investigation history.</p>
      </header>

      <nav className="scenario-nav" aria-label="Evidence scenarios">
        <a href="#case-workspace">New case<span>Identity and triage</span></a>
        <a href="#active-case-control">Active case<span>Metadata and lifecycle</span></a>
        <a href="#conditional_access">Conditional Access<span>CSV sign-in evidence</span></a>
        <a href="#resource_assignment">Resource assignment<span>Entitlement evidence</span></a>
        <a href="#guest_b2b">Guest / B2B<span>Lifecycle evidence</span></a>
        <a href="#history">History<span>Cases, runs and exports</span></a>
      </nav>

      <aside className="privacy-note" aria-label="Evidence safety guidance">
        <strong>Use redacted evidence and metadata only.</strong>
        Replace real names, email addresses, tenant IDs, object IDs, tokens, confidential resource names, and sensitive ticket content before entry.
      </aside>
      ...
      <section id="case-workspace" className="case-workspace" aria-labelledby="case-title">
        <div className="section-heading"><span>Investigation intake</span><h2 id="case-title">Create a persisted operational case</h2></div>
        <form onSubmit={createCase} className="case-form case-intake-grid">
          ...
          <div><label htmlFor="case-priority">Priority</label><select id="case-priority" value={casePriority} onChange={(event) => setCasePriority(event.target.value as CasePriority)}><option value="low">Low</option><option value="normal">Normal</option><option value="high">High</option><option value="critical">Critical</option></select></div>
          <div><label htmlFor="case-reference">Redacted external reference</label><input id="case-reference" value={caseReference} onChange={(event) => setCaseReference(event.target.value)} maxLength={80} /></div>
          <div className="wide-field"><label htmlFor="case-summary">Redacted case summary</label><textarea id="case-summary" rows={3} value={caseSummary} onChange={(event) => setCaseSummary(event.target.value)} maxLength={500} /></div>
          <button type="submit" disabled={unavailable}>{loading ? "Creating case…" : "Create investigation"}</button>
        </form>
      </section>

      <section id="active-case-control" className="case-control" aria-labelledby="active-case-title">
        <div className="section-heading"><span>Operational control</span><h2 id="active-case-title">Active investigation</h2></div>
        {activeCase ? (
          <>
            <div className={`active-case priority-${activeCase.priority}`} role="status">
              <span>Active investigation</span>
              <strong>{activeCase.title}</strong>
              <code>{activeCase.investigation_id}</code>
              <small>{activeCase.scenario_type} · {activeCase.status} · {activeCase.analysis_run_count} run(s)</small>
              <div className="case-badges"><b>{priorityLabel(activeCase.priority)} priority</b>{activeCase.external_reference && <b>{activeCase.external_reference}</b>}</div>
              {activeCase.summary && <p>{activeCase.summary}</p>}
            </div>
            <form onSubmit={updateCaseMetadata} className="metadata-form">
              <div><label htmlFor="edit-title">Case title</label><input id="edit-title" value={editTitle} onChange={(event) => setEditTitle(event.target.value)} minLength={3} required /></div>
              <div><label htmlFor="edit-priority">Priority</label><select id="edit-priority" value={editPriority} onChange={(event) => setEditPriority(event.target.value as CasePriority)}><option value="low">Low</option><option value="normal">Normal</option><option value="high">High</option><option value="critical">Critical</option></select></div>
              <div><label htmlFor="edit-reference">Redacted external reference</label><input id="edit-reference" value={editReference} onChange={(event) => setEditReference(event.target.value)} maxLength={80} /></div>
              <div className="wide-field"><label htmlFor="edit-summary">Redacted case summary</label><textarea id="edit-summary" rows={4} value={editSummary} onChange={(event) => setEditSummary(event.target.value)} maxLength={500} /></div>
              <div className="case-actions"><button type="submit" disabled={unavailable}>Save case metadata</button>{lifecycleAction && <button type="button" className="secondary" disabled={unavailable} onClick={() => transitionCase(lifecycleAction.status)}>{lifecycleAction.label}</button>}</div>
            </form>
          </>
        ) : <p className="empty-state">No active investigation. Create a case above or open one from history.</p>}
      </section>
      ...
      <section id="history" className="history-panel" aria-labelledby="history-title">
        <div className="section-heading"><span>Case register</span><h2 id="history-title">Investigation history</h2></div>
        ...
        {historyLoading ? <p className="status" role="status">Loading local investigation history…</p> : investigations.length === 0 ? <div className="empty-state"><strong>No persisted investigations yet.</strong><span>Create the first case in the investigation workspace.</span></div> : <ul className="history-list">{investigations.map((item) => <li key={item.investigation_id} className={activeCase?.investigation_id === item.investigation_id ? "is-active" : ""}><button type="button" onClick={() => activateCase(item)}>{item.title}</button><div className="history-meta"><span>{item.status} · {item.analysis_run_count} run(s)</span><small>{item.scenario_type} · {priorityLabel(item.priority)} priority</small>{item.external_reference && <small>{item.external_reference}</small>}{item.summary && <small>{item.summary}</small>}</div><button type="button" className="secondary" onClick={() => loadHistory(item.investigation_id)}>Runs</button><button type="button" className="secondary" onClick={() => changeArchiveState(item.investigation_id, item.status === "archived" ? "reopen" : "archive")}>{item.status === "archived" ? "Reopen" : "Archive"}</button></li>)}</ul>}
        {selectedId && <section className="runs-panel" aria-labelledby="runs-title"><h3 id="runs-title">Analysis runs for {selectedId}</h3>{runs.length === 0 ? <p>No analysis runs yet. This case is ready for evidence.</p> : <ul>{runs.map((run) => <li key={run.run_number}><strong>Run {run.run_number}</strong> — {run.ruleset_version}, {run.finding_count} finding(s){" "}<a href={`/api/investigations/${selectedId}/runs/${run.run_number}/report.json`}>Export JSON</a>{" · "}<a href={`/api/investigations/${selectedId}/runs/${run.run_number}/report.md`}>Export Markdown</a></li>)}</ul>}</section>}
      </section>
    </main>
  );
}
